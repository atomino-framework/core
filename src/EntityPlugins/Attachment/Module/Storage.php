...
use Atomino\EntityPlugins\Attachment\AttachmentPlugin;
 ...
 * @property-read \Atomino\EntityPlugins\Attachment\Module\Attachment[] $attachments
 * @property-read \Atomino\EntityPlugins\Attachment\Module\Collection[] $collections
 */
class Storage implements \JsonSerializable{

	/** @var \Atomino\EntityPlugins\Attachment\Module\Collection[] */
	private array $collections = [];
	private string $path = '';
	private string $url;
	/** @var \Atomino\EntityPlugins\Attachment\Module\Attachment[] */
	...
	public function __construct(private Entity|EntityAttachmentInterface $entity, array $attachments){
		...
		// Create Collections
		foreach (AttachmentPlugin::fetch($entity::model())->getCollections() as $collection){
	...
	public function addFile(File $file): string{
		...
		$this->attachments[$filename] = new Attachment($this,
																	  $file->getFilename(),
																	  $file->getSize(),
																	  $file->getMimeType(),
																	  '',
	...
	public function rename(string $filename, string $newName):void{
		...
		$this->begin();
		$this->getAttachment($filename)->deleteImages();
